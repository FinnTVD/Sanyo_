    ...
    <section className='relative pt-[7.5rem] max-md:pt-[2.37rem] max-md:border-t max-md:border-[rgba(28, 32, 28, 0.10)]'>
      ...
      <div className='pl-[6.31rem] flex items-end justify-between pr-[3.56rem] max-md:px-3'>
        <div className='w-[55.625rem]'>
          ...
          >
            <h2
              className='text-[3.125rem] font-SVNLagu font-semibold leading-1.3 text-grey-900 mb-4 max-md:text-2xl max-md:leading-1.3 [&>p>strong]:font-semibold  max-md:mb-[0.62rem] [&>p>strong]:text-c-nht max-md:[&>p]:flex max-md:[&>p]:flex-col'
              dangerouslySetInnerHTML={{
                __html: dataListEle?.title || 'Thang máy Nhật Bản SANYO YUSOKI',
              }}
            ></h2>
          </Fade>
          <Fade
          ...
          >
            <span
              className='text-[1.125rem] font-Iciel leading-1.7 font-normal uppercase text-[#42484F] [&>p>strong]:text-c-nht max-md:text-[0.75rem]'
              dangerouslySetInnerHTML={{
                __html:
                  dataListEle?.desc ||
      ...
      >
        {!isMobile ? (
          <SliderJapanElevator data={dataListEle?.elavators} />
        ) : (
          <SliderMbJapanElevator data={dataListEle?.elavators} />
        )}
      </Fade>
    </section>
  )
