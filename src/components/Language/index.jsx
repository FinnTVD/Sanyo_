  ...
  return (
    <div>
      ...
      <div className='flex items-center space-x-[0.56rem] cursor-pointer relative fatherLang w-[7.95rem] max-md:space-x-[0.19rem]'>
        ...
        />
        <span className='max-md:text-[0.75rem] text-grey-700 font-Iciel font-medium leading-1.4 text-[1rem] notranslate'>
          {isMobile
            ? currentLanguage == 'vi'
              ? 'VN'
              : 'ENG'
        ...
        <div className='absolute top-full left-[-1rem]  flex flex-col bg-white rounded-xl px-4 py-3 childLang shadow-lg w-[10rem] max-md:w-full'>
          <label
            className='flex items-center max-md:mb-[0.5rem] mb-3'
            onClick={() => handleLanguageChange('en')}
          >
            <div
              className='size-4 border border-yellow-500 rounded-[50%] mr-2 flex items-center justify-center'
              onClick={() => handleLanguageChange('en')}
            >
              <div
                className={`size-[0.8rem] bg-yellow-500 rounded-[50%] ${
                  currentLanguage === 'vi' ? 'hidden' : ''
                }`}
              ></div>
            </div>
            <span className='font-Iciel whitespace-nowrap font-normal text-[1rem] leading-1.5 max-md:text-[0.75rem]'>
              English
            </span>
          </label>
          <label
